...
<template>
  ...
  <div v-else class="space-y-6">
    <div v-for="m in milestones" :key="m.id" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      <div class="p-4 sm:p-5 cursor-pointer" @click="toggle(m.id)">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div class="flex items-center gap-3 min-w-0">
            <span class="w-8 h-8 rounded-lg bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-sm shrink-0">{{ m.monthNo }}</span>
            <div class="min-w-0">
              <h3 class="font-bold text-slate-800 truncate">{{ m.title }}</h3>
      ...
      <div v-if="expanded[m.id]" class="border-t border-slate-100 bg-slate-50 p-4 sm:p-5">
        <h4 class="text-xs font-semibold text-slate-500 uppercase mb-3">태스크 목록 ({{ (tasksByMilestone[m.id] || []).length }}건)</h4>
        <div class="space-y-2">
          <div v-for="t in tasksByMilestone[m.id]" :key="t.id" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 bg-white rounded-xl border border-slate-100">
