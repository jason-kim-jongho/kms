    ...
    <div v-else-if="detail" class="space-y-6">
      <section class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div class="flex items-start justify-between mb-4">
          <div>
            <h2 class="text-xl font-bold text-slate-800">{{ detail.document.title }}</h2>
            <p class="text-xs text-slate-400 font-mono mt-1">{{ detail.document.storageId }}</p>
          </div>
      ...
      <section class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <button @click="triggerUpload" :disabled="uploading" class="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
            <i class="fas fa-upload mr-1"></i>{{ uploading ? '업로드 중...' : '파일 업로드' }}
          </button>
          <input ref="fileInput" type="file" class="hidden" @change="onFileSelected" />
        </div>
        <div v-if="detail.files.length" class="space-y-2">
          <div v-for="f in detail.files" :key="f.id" class="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50">
            <div class="flex items-center gap-3">
              <i class="fas fa-file text-slate-400"></i>
              <div>
                <span class="text-sm font-medium text-slate-700">{{ f.originalFileName }}</span>
      ...
      <section class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        ...
        <div v-if="detail.sap_links.length" class="space-y-2">
          <div v-for="l in detail.sap_links" :key="l.id" class="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50">
            <div>
      ...
      <!-- Certifications -->
      <section v-if="detail.certifications.length" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h3 class="font-bold text-slate-800 mb-4"><i class="fas fa-certificate mr-2 text-orange-600"></i>연결된 인증서</h3>
        <div class="space-y-2">
          <div v-for="c in detail.certifications" :key="c.id" class="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50">
            <div>
