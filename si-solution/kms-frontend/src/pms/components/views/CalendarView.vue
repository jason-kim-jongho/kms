...
<template>
  <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-3 sm:px-4 py-3 border-b border-slate-100">
      <div class="flex items-center gap-3 order-1 sm:order-none">
        ...
        <p class="font-bold text-slate-800 sm:hidden">{{ cursor.format('YYYY년 M월') }}</p>
      </div>
      <p class="font-bold text-slate-800 hidden sm:block">{{ cursor.format('YYYY년 M월') }}</p>
      <p class="text-xs text-slate-400 truncate">기준: {{ meta.fields.find(f => f.key === dateFieldKey)?.label || dateFieldKey }}</p>
    </div>
      ...
      <div class="min-w-[560px]">
        ...
        <div class="grid grid-cols-7">
          <div
            ...
            :key="cell.key"
            class="min-h-[70px] sm:min-h-[92px] border-b border-r border-slate-50 p-1 sm:p-1.5"
