...
<template>
  <div class="bg-white rounded-2xl border border-slate-200 overflow-x-auto">
    <div class="divide-y divide-slate-50 min-w-[640px]">
      <div class="flex items-center gap-4 px-4 py-2.5 text-xs text-slate-400 uppercase font-medium">
        <input type="checkbox" class="rounded" />
        <span class="flex-1 min-w-[140px]">name</span>
        <span v-for="f in linkedFields" :key="f.key" class="w-40 shrink-0">{{ f.label }}</span>
        <span v-for="f in otherFields" :key="f.key" class="w-28 shrink-0">{{ f.label }}</span>
        <span v-if="noteField" class="w-40 shrink-0">{{ noteField.label }}</span>
      </div>
      ...
      >
        ...
        <span class="flex-1 min-w-[140px] text-sm font-medium text-slate-700 truncate">{{ rec[meta.titleField] }}</span>

        <span v-for="f in linkedFields" :key="f.key" class="w-40 shrink-0 truncate">
        ...
        <span v-for="f in otherFields" :key="f.key" class="w-28 shrink-0 text-xs">
          <FieldBadge v-if="f.type === 'select'" :value="rec[f.key]" :options="f.options" />
          <span v-else class="text-slate-500">{{ rec[f.key] ?? '-' }}</span>
        </span>

        <span v-if="noteField" class="w-40 shrink-0 text-xs text-slate-400 truncate">{{ rec[noteField.key] || '-' }}</span>
      </div>
